...
<template>
  <div class="cart">
    <RouterLink to="/cart" class="curr">
      <i class="iconfont icon-cart"></i><em>{{ cartStore.validTotal }}</em>
    </RouterLink>
    <!-- 弹出层 -->
    <!-- 没有商品不显示，并且不是在购物车页面 -->
    <div class="layer" v-if="cartStore.validTotal && route.path !== '/cart'">
      <div class="list">
      ...
      <div class="foot">
        <div class="total">
          ...
          <p>&yen;{{ cartStore.validAmount }}</p>
        </div>
        <WebShopButton type="plain" @click="router.push('/cart')">去购物车结算</WebShopButton>
      </div>
    </div>
  </div>
</template>
